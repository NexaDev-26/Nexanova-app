...
router.get('/profile', verifyToken, (req, res) => {
  db.get('SELECT id, nickname, email, path, ai_personality, mood_score, anonymous_mode, offline_mode, store_chat, language, total_points, level, created_at FROM users WHERE id = ?', [req.userId], (err, user) => {
...
router.patch('/profile', verifyToken, (req, res) => {
  const updates = [];
  const params = [];
  ['nickname', 'path', 'ai_personality', 'offline_mode', 'store_chat', 'mood_score', 'language'].forEach((f) => {
  ...
  db.run(`UPDATE users SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, params, function(err) {
